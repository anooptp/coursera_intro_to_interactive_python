...
class Sprite:
    ...
    def draw(self, canvas):
        ...
        if self.animated:
            if time % 2==0:
                self.rock_index += 1
                exp_center, exp_size = explosion_info.get_center(), explosion_info.get_size()
                canvas.draw_image(explosion_image, 
                                  [exp_center[0] + self.rock_index * exp_size[0], 
                                   exp_center[1]],
                                   self.image_size, self.pos, self.image_size)
            if self.rock_index == 24:
...
def group_group_collide(group1, group2):
    coll_count=0
    for ele in list(group1):
        if group_collide(group2, ele):
            coll_count += 1
            group1.discard(ele)
    return coll_count
...
def draw(canvas):
    ...
    if started and group_collide(rock_group, my_ship) and lives > 0:
        lives -= 1
    if started and (not lives):
        started=False
        rock_group = set()
        soundtrack.rewind()
...
def rock_spawner():
    if started:
        ...
        if len(rock_group)< 12:
            ...
            if dist(my_ship.get_position(), new_rock.get_position()) > my_ship.get_radius() + new_rock.get_radius() + 30:
